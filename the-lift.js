...
// The floor they want to go to is represented by a number (i.e. when they enter the Lift
//     this is the button they will press)
...
// The Lift never changes direction until there are no more people wanting to get on/off in
// the direction it is already travelling
...
// NOTE: The Lift always starts on the ground floor (and people waiting on the ground floor
...
// Parameter validation - All input parameters can be assumed OK. No need to check for things
// like:
...
var theLift = function(queues, capacity) {
  ...
  const goUp = startFloor => {
    for (i = startFloor; i <= queues.length + 1; i++) {
      // Check if there is a queue, if not, move to next floor
